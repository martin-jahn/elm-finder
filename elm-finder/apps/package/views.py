...
def edit_package(request, slug, template_name="package/package_form.html"):
    ...
    return render(
        request, template_name, {"form": form, "package": package, "repo_data": repo_data_for_js(), "action": "edit"}
    )
...
def add_example(request, slug, template_name="package/add_example.html"):
    ...
    if form.is_valid():
        package_example = PackageExample(
            package=package, title=request.POST["title"], url=request.POST["url"], created_by=request.user
        )
...
def edit_example(request, slug, id, template_name="package/edit_example.html"):
    ...
    return render(request, template_name, {"form": form, "package_example": package_example})
...
def ajax_package_list(request, template_name="package/ajax_package_list.html"):
    ...
    return render(
        request, template_name, {"packages": packages, "packages_already_added_list": packages_already_added_list}
    )
...
def usage(request, slug, action):
    ...
    next = (
        request.GET.get("next") or request.META.get("HTTP_REFERER") or reverse("package", kwargs={"slug": package.slug})
    )
