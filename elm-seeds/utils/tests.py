...
def load():
    category, created = Category.objects.get_or_create(
        pk=1, slug="apps", title="App", description="Small components used to build projects."
    )
    ...
    grid1, created = Grid.objects.get_or_create(
        pk=1, description="A grid for testing.", title="Testing", is_locked=False, slug="testing"
    )
    grid2, created = Grid.objects.get_or_create(
        pk=2, description="Another grid for testing.", title="Another Testing", is_locked=False, slug="another-testing"
    )

    gridpackage1, created = GridPackage.objects.get_or_create(pk=1, package=package1, grid=grid1)
    gridpackage2, created = GridPackage.objects.get_or_create(pk=2, package=package1, grid=grid1)
    gridpackage3, created = GridPackage.objects.get_or_create(pk=3, package=package3, grid=grid1)
    gridpackage4, created = GridPackage.objects.get_or_create(pk=4, package=package3, grid=grid2)
    gridpackage5, created = GridPackage.objects.get_or_create(pk=5, package=package2, grid=grid1)

    feature1, created = Feature.objects.get_or_create(
        pk=1, title="Has tests?", grid=grid1, description="Does this package come with tests?"
    )
    feature2, created = Feature.objects.get_or_create(
        pk=2, title="Coolness?", grid=grid1, description="Is this package cool?"
    )

    element, created = Element.objects.get_or_create(pk=1, text="Yes", feature=feature1, grid_package=gridpackage1)
    ...
    group1.permissions.set(
        [Permission.objects.get(codename="delete_gridpackage"), Permission.objects.get(codename="delete_feature")]
    )
    ...
    packageexample, created = PackageExample.objects.get_or_create(
        pk=1, package=package1, url="http://www.example.com/", active=True, title="www.example.com"
    )

    packageexample2, created = PackageExample.objects.get_or_create(
        pk=2, package=package1, url="http://my.example.com/", active=True, title="my.example.com", created_by=user1
    )
